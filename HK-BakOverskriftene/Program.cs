...
//    }
//}
using BakOverskriftene.Domain.Models; // Ensure this namespace is included
using HK_BakOverskriftene;
using Microsoft.AspNetCore.Identity;
using Microsoft.EntityFrameworkCore;
...
internal class Program {
    private static void Main(string[] args) {
        ...
        // Add Identity services and register the DbContext
        builder.Services.AddIdentity<Player, IdentityRole>()  // Use Player model
            .AddEntityFrameworkStores<ApplicationDbContext>()
            .AddDefaultTokenProviders();
        ...
        // Call the SeedData method to add dummy players
        using (var scope = app.Services.CreateScope()) {
            var services = scope.ServiceProvider;
            var userManager = services.GetRequiredService<UserManager<Player>>();  // Correct type for UserManager
            var roleManager = services.GetRequiredService<RoleManager<IdentityRole>>();

            // Seed the data (add users if they do not exist)
